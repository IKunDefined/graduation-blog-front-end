    ...
    <div class="main">
      <div class="left">
          ...
          <mu-list textline="three-line" style="padding: 0; box-shadow: 0 0 1px 1px #ccc;">
            <mu-list-item avatar :ripple="false" button style="background: #fff;">
              ...
              <mu-list-item-content>
                <mu-list-item-title>{{ message.username }}</mu-list-item-title>
        ...
        <div class="comment" v-if="isLogin">
          <div class="comment-main" v-if="message.comment.length !== 0">
            <mu-expansion-panel v-for="(item, index) in message.comment" :key="index">
              <div slot="header">{{ item.username }}</div>
              {{ item.content }}
            </mu-expansion-panel>
          </div>
          <div class="no-content" v-if="message.comment.length === 0">还没有评论，来抢沙发吧！</div>
...
export default {
  ...
  methods: {
    ...
    leaveComment () {
      ...
      comment.messageId = this.messageId
      axios.post(`http://localhost:4000/blog/api/message/comment/leave`, { comment }).then(res => {
        if (res.data.code === 0) {
          this.comment = ''
          this.getMessage()
        }
